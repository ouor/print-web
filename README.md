...
`PRINTER_NAME`이 콤마로 여러 개 나열돼 있으면 위 절차를 **프린터마다 독립적으로** 실행한다. 한 대가 캘리브레이션에 실패해도(오프라인 등) 다른 대는 계속 진행되고, 실패한 프린터는 워커가 매 잡마다 per-call probe로 GDI에 직접 묻는다(오버사이즈 트릭 미적용). 각 프린터의 DEVMODE 스냅샷은 따로 보관됐다가 종료 시 모두 복원된다.
...
# 단일 프린터: 이름 한 개. 여러 대 동시 운용: 콤마로 나열 (워커가 프린터마다
# 하나씩 떠서 유휴한 프린터로 잡을 라우팅). 비우면 시작 시 대화형 선택,
# TTY 없으면 Windows 기본으로 폴백.
PRINTER_NAME=
# PRINTER_NAME=Samsung CLX-8380 Series PS,HP Business Inkjet 3000 PS
...
- **프린터당 워커 1개, 유휴 프린터로 자동 라우팅.** `PRINTER_NAME`이 `A,B,C`처럼 콤마 나열되면 `app/main.py`가 부팅 시 각각을 캘리브레이션하고 `worker_loop` 태스크를 N개 띄운다. 모든 워커는 동일한 `asyncio.Lock` 아래서 `_claim_next_job`을 호출하므로 두 워커가 같은 행을 잡는 경쟁이 없고, **방금 한 잡을 마친 워커가 다음 락 획득자가 되니** "유휴 프린터로 라우팅"이 자연스럽게 emerge한다. 같은 시각에 5건을 승인하면 N대가 병렬로, 잡 하나는 여전히 한 프린터에서 한 번에 한 장씩 처리된다. 워커가 잡을 claim할 때 `Job.printer_name`을 세팅해 어느 프린터로 갔는지 감사 흔적을 남긴다.
