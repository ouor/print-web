...
- 백엔드는 6단계를 그대로 노출하지만, **공개 응답(`PublicJob`)에서는 `reject_reason`을 빼고** 클라이언트의 사용자 뷰는 3단계로 축약한다:

| 사용자 뷰 라벨 | 매핑되는 상태 |
|---|---|
| 대기중 | PENDING, APPROVED |
| 진행중 | PRINTING |
| 완료 | DONE, FAILED, REJECTED (서브 카피로만 결과 차이 시사) |

관리자의 존재나 거절 사실은 사용자 화면에 절대 드러내지 않는다.
...
- **단일 워커, 직렬 인쇄.** 프린터는 한 번에 한 장씩이 자연스럽고, 동시 인쇄로 얻을 게 없다. `app/printer/worker.py`는 가장 오래된 `APPROVED` 작업을 atomic하게 `PRINTING`으로 마킹하고 GDI 호출을 스레드 풀에 넘긴 뒤 `_mark_done` / `_mark_failed`로 정산한다.
